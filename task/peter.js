...
router.get("/education/noneRegisterSubject", (req, res) => {
  db.query("select S.id,S.idsubject,S.name,S.credit,Sc.name as 'category_name' from subjects S join subject_category Sc on S.subject_category_id=Sc.id where not exists (SELECT 1 FROM subjectsRegister Sr WHERE Sr.Subjects_id = S.id) and S.isopen=1", (err, results) => {
    if (err) {
      res.status(500).json({ msgerror: "Database Error:" + err });
      return;
    } else {
  ...
  return new Promise((resolve, reject) => {
    ...
    Promise.allSettled(
      ...
      .then(() => {
        if (errors.length === 0) {
          resolve({
            data: "Data successfully imported", warn: { data: warn, warnmsg: warn.length === 0 ? null : `${warn.length} วิชาที่มีการลงทะเบียนก่อนหน้าแล้ว`, },
          });
        } else {
          // Provide detailed error information
          reject({
            data: `${errors.length} errors occurred during import`, errors: errors, warn: { data: warn, warnmsg: warn.length === 0 ? null : `${warn.length} วิชาที่มีการลงทะเบียนก่อนหน้าแล้ว`, },
          });
        }
      })
      .catch((err) => {
...
router.post("/teacher/registersubject", (req, res) => {
  ...
  db.query("select * from timeSystem", (err, result) => {
    ...
    const InsertDatabase = new Promise((resolve, reject) => {
      ...
      Promise.allSettled(errors.map((error) => new Promise((resolve) => db.query(`ROLLBACK`, resolve))).concat([new Promise((resolve) => setTimeout(resolve, 1000))])
      ).then(() => {
        if (errors.length === 0) {
            ...
            db.query(
              ...
              (err, results) => {
                if (err) {
                  ...
                  if (results.length > 0) {
                    res.status(200).json({
                      results, msgtime: `ไม่ได้เปิดระบบให้ลงทะเบียน ระบบเปิด ${data.toLocaleString("th-th", { year: "numeric", day: "2-digit", month: "long", hour: "2-digit", minute: "2-digit", })} ถึง ${data1.toLocaleString("th-th", { year: "numeric", day: "2-digit", month: "long", hour: "2-digit", minute: "2-digit", })}`,
                    });
                  } else {
                    res.status(200).json({
                      msg: "ไม่มีวิชาที่เปิดสอน", msgtime: `ไม่ได้เปิดระบบให้ลงทะเบียน ระบบเปิด ${data.toLocaleString("th-th", { year: "numeric", day: "2-digit", month: "long", hour: "2-digit", minute: "2-digit", })} ถึง ${data1.toLocaleString("th-th", { year: "numeric", day: "2-digit", month: "long", hour: "2-digit", minute: "2-digit", })}`,
                    });
                  }
                }
              }
  ...
  db.query("select status,S_date,E_date,S_time,E_time,type from timeSystem where id =1", (err, results) => {
    if (err) {
      ...
      if (results[0].status === 0) {
        ...
        db.query("select S.name,S.idsubject,S.credit,S.practice_t,S.lecture_t,S.years,Sr.name as subject_category,S.m_t  ,S.exsub from subjects S join subject_category Sr on Sr.id=S.subject_category_id  where IsOpen=1 and S.id=?", [id], (err, results) => {
          if (err) {
            console.log(err);
