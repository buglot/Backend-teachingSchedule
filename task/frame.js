...
router.get("/user", (req, res) => {
  const sql =
    "select user.id,user.email,user.name,user.role_id,role.name as rolename from user,role where user.role_id = role.id order by id;";
  db.query(sql, (err, results) => {
    if (err) {
      console.error("Error executing SELECT statement:", err);
      res.status(500).json({ error: "Internal Server Error" });
...
router.post("/savefiledate", (req, res) => {
  ...
  const sql =
    "INSERT INTO user (date,filename,link,type,years) VALUES (?,?,?,?,?)";
...
router.post("/ubdatesubjectsRegister", (req, res) => {
  ...
  const sql =
    "UPDATE subjectsRegister SET (day_id,st,et,status_id) VALUES (?,?,?,?) where id=?";
...
router.get("/searchregister/:search", (req, res) => {
  ...
  const sql =
    "SELECT distinct S.idsubject, S.name FROM subjects S WHERE S.idsubject LIKE ? OR S.name LIKE ? and isOpen=1";
  ...
  db.query(sql, [`%${search}%`, `%${search}%`], (err, results) => {
    ...
    db.query(sql1, [`%${search}%`], (err, result) => {
      ...
      if (data.length === 0) {
        res
          .status(401)
          .json({ error: "No data found for the provided search term" });
      } else {
...
router.post("/eu/ubdatestatusregister", (req, res) => {
  const { id } = req.body;
  const { st, et, day_id } = req.body;
  if (st && et) {
    const sql =
      "UPDATE subjectsRegister SET status_id= 1 , st=?,et=?,day_id=? where id=?";
    db.query(sql, [st, et, day_id, id], (err, result) => {
      if (err) {
        console.log(err);
        return res.status(500).send("Error updating subject register");
      } else {
    ...
    const sql = "UPDATE subjectsRegister SET status_id=1 where id=?";
